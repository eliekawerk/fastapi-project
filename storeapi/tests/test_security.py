...
def test_create_confirmation_token():
    ...
    assert {"sub": "123", "type": "confirmation"}.items() <= security.jwt.decode(
        token, key=security.SECRET_KEY, algorithms=[security.ALGORITHM]
    ).items()
...
def test_subject_for_token_type_expired(mocker):
...
def test_subject_for_invalid_token(mocker):
    token = "bogus token"
    with pytest.raises(security.HTTPException) as exc_info:
        security.get_subject_for_token_type(token, "access")
...
async def test_authenticate_user_wrong_password(registered_user: dict):
    with pytest.raises(security.HTTPException):
        await security.authenticate_user(registered_user["email"], "blabla")
